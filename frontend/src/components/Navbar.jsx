...
const Navbar = () => {
    ...
    return (
        <nav style={{ padding: '1rem', borderBottom: '1px solid #ccc', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
                <Link to="/" style={{ marginRight: '1rem', fontWeight: 'bold', textDecoration: 'none', color: '#333' }}>Manejador de Eventos</Link>
            </div>
            <div style={{ padding: '1rem' }}>
                {user ? (
                    <>
                        <span style={{ marginRight: '1rem' }}>Hola, {user.name} Permisos: {user.role}</span>
